...
export default function EditBill() {
  ...
  useEffect(() => {
    if (profile?.role !== 'admin') { navigate('/bills'); return; }
    async function fetchBill() {
      const { data, error } = await supabase.from('bills').select('*').eq('id', id).single()
      if (error || !data) { setError('Bill not found.'); setFetchLoading(false); return; }
      setOriginalValues(data)
      setForm({
        bill_number: data.bill_number || '',
        bill_type: data.bill_type || 'table',
        table_number: data.table_number || '',
        customer_note: data.customer_note || '',
        total_amount: String(data.total_amount ?? ''),
        discount: String(data.discount ?? '0'),
        tip_amount: String(data.tip_amount ?? ''),
        payment_status: data.payment_status || 'paid',
        payment_method: data.payment_method || 'cash',
        cash_amount: String(data.cash_amount ?? ''),
        card_amount: String(data.card_amount ?? ''),
        paypal_amount: String(data.paypal_amount ?? ''),
        notes: data.notes || '',
        bill_date: localDateTimeInputValueFromValue(data.bill_date),
      })
  ...
  const validateForm = () => {
    ...
    if (!form.total_amount || total < 0) return 'Total amount must be 0 or more.'
    if (discount > total) return 'Discount cannot be more than total amount.'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data: updated, error: updateError } = await supabase.from('bills').update({
      ...
      if (!updated || updated.length === 0) throw new Error('Update failed: you may not have permission to edit this bill.')

      await supabase.from('audit_logs').insert({
        action: 'UPDATE',
        table_name: 'bills',
        performed_by: user?.id,
        old_data: { bill_number: originalValues?.bill_number, total_amount: originalValues?.total_amount },
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-800">Edit Bill</h1>
        ...
        {success && (
          <div className="bg-green-50 border border-green-200 text-green-700 rounded-lg px-4 py-3 mb-6 text-sm">{success}</div>
        )}
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input name="bill_number" value={form.bill_number} onChange={handleChange} required
                className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Date & Time</label>
              <input type="datetime-local" name="bill_date" value={form.bill_date} onChange={handleChange} required
                className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {['table', 'parcel', 'abholung'].map((type) => (
                <button key={type} type="button" onClick={() => setForm({ ...form, bill_type: type })}
                  className={`py-3 rounded-lg border-2 text-sm font-medium capitalize transition-colors ${
                    form.bill_type === type ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'
                  }`}>
          ...
          {form.bill_type === 'table' && (
            <div>
              ...
              <input name="table_number" value={form.table_number} onChange={handleChange} placeholder="e.g. Table 5"
                className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          )}
          ...
          <div>
            ...
            <input name="customer_note" value={form.customer_note} onChange={handleChange}
              className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
                  ...
                  {['cash', 'card', 'paypal', 'mixed'].map((method) => (
                    <button key={method} type="button" onClick={() => setForm({ ...form, payment_method: method })}
                      className={`py-2.5 rounded-lg border-2 text-sm font-medium capitalize transition-colors ${
                        form.payment_method === method ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'
                      }`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {(form.payment_method === 'cash' || form.payment_method === 'mixed') && (
                  <div>
                    ...
                    <input type="number" name="cash_amount" value={form.cash_amount} onChange={handleChange}
                      min="0" step="0.01" placeholder="0.00"
                      className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                )}
                {(form.payment_method === 'card' || form.payment_method === 'mixed') && (
                  <div>
                    ...
                    <input type="number" name="card_amount" value={form.card_amount} onChange={handleChange}
                      min="0" step="0.01" placeholder="0.00"
                      className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                )}
                {(form.payment_method === 'paypal' || form.payment_method === 'mixed') && (
                  <div>
                    ...
                    <input type="number" name="paypal_amount" value={form.paypal_amount} onChange={handleChange}
                      min="0" step="0.01" placeholder="0.00"
                      className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                )}
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            ...
            <div className="relative">
              ...
              <input type="number" name="tip_amount" value={form.tip_amount} onChange={handleChange}
                min="0" step="0.01" placeholder="0.00"
